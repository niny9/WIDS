...
import os
os.chdir("/kaggle/input/d/tingz0518/utils2/features/")
from treat_data_for_autoglun_wmy import treat_data_autoglun
...
class EnsembleModel():
    ...
    def fit(self,X,X_GCN,y):
        ...
        train_loader = DataLoader(X_GCN, batch_size=32, shuffle=True)
        ...
        self.model4 = self.fit_GCN(train_loader)
        self.model4.eval()
        model4_pred = []
        with torch.no_grad():
            for data in tqdm(train_loader, desc="模型评估"):
                out = self.model4(data)
                probs = torch.sigmoid(out)  # 获取概率值
                model4_pred.extend(probs[:, 0].tolist())
    ...
    def predict(self, X_test, X_test_GCN, y_test):
        ...
        with torch.no_grad():
            for data in tqdm(test_loader, desc="模型评估"):
                out = self.model4(data)
                probs = torch.sigmoid(out)  # 获取概率值
                target = data.y.view(-1, 1)  # 同样需要重塑维度
                
                model4_labels.extend(target[:, 0].int().tolist())
                model4_output.extend(probs[:, 0].tolist())
