...
def prepare_for_trainning(train_solutions, train_connectome, train_cat,test_solutions,test_connectome, test_cat, goal):
    ...
    def treat_connectome(train_solutions,train_connectome,train_metadata):
        ...
        def load_connectomes(df_conn, df_soln, as_vectors=False):
            ...
            # 初始化优化后的斜对称矩阵空间
            skew_space = SkewSymmetricMatrices(200, equip=False)
            skew_space.dtype = gs.float32  # 强制指定数据类型
            ...
            if 'mat' in locals():
                ...
                if os.path.exists(temp_path):
                    os.remove(temp_path)  # 可选：删除临时文件
                    
            temp_dir = Path(__file__).parent.resolve() / "temp"  # 创建专用临时目录
            temp_dir.mkdir(exist_ok=True, parents=True)
            temp_path = str(temp_dir / f"temp_matrix_{uuid.uuid4().hex}.dat")  # 每次生成唯一文件名
            print(temp_path)
            #os.makedirs(os.path.dirname(temp_path), exist_ok=True)
            # 创建内存映射文件
            try:
                mat = np.memmap(temp_path, dtype=np.float32, mode='w+', shape=(n_samples, 200, 200))
            except OSError as e:
                print(f"错误详情: {e}")
                print(f"绝对路径有效性: {Path(temp_path).exists()}")
                print(f"父目录权限: {os.access(Path(temp_path).parent, os.W_OK)}")
                raise
            #mat = np.memmap(temp_path, dtype=np.float32, mode='w+', shape=(n_samples, 200, 200))
            ...
            batch_size = min(1000, int(available_mem / (200 * 200 * 4) * 0.5))  # 按可用内存50%计算
            ...
            for i in range(0, n_samples, batch_size):
                ...
                # 生成斜对称矩阵
                batch = data[i:i+batch_size]
                mat_batch = skew_space.matrix_representation(batch)
                mat_batch = gs.eye(200, dtype=gs.float32) - gs.transpose(gs.tril(mat_batch), (0,2,1))
                
                # 对称化处理
                sym_batch = 0.5 * (mat_batch + gs.transpose(mat_batch, (0,2,1)))
                mat[i:i+batch_size] = sym_batch
                
                # 及时释放内存
                del mat_batch, sym_batch
                gc.collect()  # 手动触发垃圾回收
        ...
        return data_corrected, data, labels, patient_id
    ...
    train_data_corrected, train_data, train_labels,train_patient_id = treat_connectome(train_solutions,train_connectome,train_metadata)
    test_data_corrected, test_data, test_labels,test_patient_id = treat_connectome(test_solutions,test_connectome,test_metadata)
    ...
    #print('train_patient_id：',train_patient_id)
    ...
    test_metadata = test_metadata.sort_values('participant_id')  # 确保元数据也按ID排序
